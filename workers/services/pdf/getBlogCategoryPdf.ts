...
export const genBlogCategoryPdf = async (email: string) => {
    const doc = new PDFDocumentWithTables({ margin: 30 });

    const outputPath = 'public/blog_category.pdf';
    ...
    try {
        ...
        doc.fontSize(20).text("Gray To Green - Blog Category", { align: "center" });
        ...
        await mailSender("Blog Category List Pdf Report", email, ["blog_category.pdf"])
    } catch (error) {
        doc.end()
        console.log(error);
    }
