...
export const genMeasurementsPdf = async (email: string) => {
    const doc = new PDFDocumentWithTables({ margin: 30 });
    const outputPath = 'public/measurements.pdf';
    ...
    try {
        ...
        doc.fontSize(20).text("Gray To Green - Measurements", { align: "center" });
        ...
        await mailSender("Measurements List Pdf Report", email, ["measurements.pdf"])
    } catch (error) {
        doc.end()
        console.log(error);
    }
